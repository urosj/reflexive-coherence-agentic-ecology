...
| `L02-Q14` | Can the N29 generator/extractor/processor operations compose with subset-bounded irreducibility through one P2-I2 shared carrier? | APP-A0–APP-A2 | Owner-accepted bounded positive: 19/19 valid arms; GEP passes while every proper subset is insufficient; each operation and the carrier satisfy the frozen causal tests; reconstruction is byte-identical | DEC-074/CHG-079 close Appendix A at fixture/gate-bounded operation complementarity through one carrier; plurality/order are non-load-bearing here |
...
| `P2-I2-CHG-086` | Owner supplies four serious pre-acceptance validation issues for APP-B2 | `appendix_b3_retained_evidence_implication_boundary_audit` | Audit frozen matched-cardinality alternatives; prove fail-open analysis isolation; reconcile H_C token admission exactly; narrow positive wording to mode-specific scaffold dependence | Retained-evidence/source inspection only; zero PyGRC/model/producer/arm/response generation; APP-B2 runtime and corrected scientific bytes immutable | DEC-080/CHG-085, owner review, committed APP-B2 freeze/runtime/correction/closeout | APP-B3 active; acceptance, commit, and any matched-cardinality runtime remain closed |
| `P2-I2-CHG-087` | APP-B3 pure retained-evidence audit completes all four review dispositions | `appendix_b3_implication_boundary_result` | Retain exact isolation/H_C reconciliation; disclose absent matched-cardinality alternative; project history-carried scaffold-dependent claim and frozen-realization negatives | 123/123 physical admissions, 111/111 final tokens, zero unexpected/duplicate/excluded admissions; fail-open post-run only; zero PyGRC/model/producer/arm/response generation | DEC-081/CHG-086, APP-B3 validator/audit/report, unchanged APP-B2 hashes | Technically complete and review-ready uncommitted; owner acceptance/commit and possible next discriminator closed |
| `P2-I2-CHG-088` | Owner directs addition of the proposed ordered three-token alternative | `appendix_b4_ordered_three_token_discriminator_entry` | Add hypothesis/checklist first; audit unchanged-baseline feasibility and budgets for all `{G,E,P}^3`; freeze 84 fresh history arms only if complete panel is natively executable | Documentation plus capability/source/budget audit only initially; zero APP-B4 model/arm/response; no silent funding, dose change, reduced panel, or APP-B2 retrofit | DEC-082/CHG-087, accepted APP-B1 freeze and APP-B2 history realization, user direction `ok, then add it` | APP-B4 capability audit active; freeze/runtime/commit and operation-complementarity claim closed |
| `P2-I2-CHG-089` | APP-B4 unchanged-baseline capability audit finds three physical budget failures | `appendix_b4_panel_feasibility_blocker_and_owner_choice` | Retain exact 27-sequence arithmetic; classify EEG/EEE/EEP operationally infeasible; return unchanged 75-arm, pre-funded 87-arm, and incomplete 84-arm options without choosing | Two pure `.venv` audit starts; 24 feasible, 3 infeasible; zero PyGRC/model/producer/arm/response; no freeze/runtime | DEC-083/CHG-088, accepted registration/baseline/routes/amounts, APP-B4 capability artifact/report | Stopped before freeze; owner panel choice required; recommendation is unchanged-baseline 75-arm panel |
| `P2-I2-CHG-090` | Owner responds `yes` to freezing the recommended option A | `appendix_b4_unchanged_baseline_75_arm_freeze_authority` | Freeze exactly 24 executable triples plus reference across three seeds; bind exclusions, accepted machinery, one-shot lifecycle, and bounded analysis; candidate-free validate and return inactive package | Freeze/validation only; zero PyGRC/model/producer/arm/response; no pre-funding, baseline/dose/admission change, or APP-B2 runtime reuse | DEC-084/CHG-089, capability artifact/report, owner selection | Inactive freeze construction active; commit/activation/runtime/result remain closed |
| `P2-I2-CHG-091` | APP-B4 exact inactive freeze construction and candidate-free validation complete | `appendix_b4_inactive_75_arm_freeze_result` | Bind deterministic 75-row registry, accepted machinery and envelope identity, transitive hashes, one-shot lifecycle, all-seed fail-closed classifiers, and explicit excluded domain; retain all construction diagnostics | Six freeze-builder starts; nine validator starts with one pre-runtime self-scan failure; one focused pure projection failure, seven absolute shebangs, and one seed-101-only classifier projection corrected before runtime; final inactive validation passes; zero PyGRC/model/producer/arm/response | DEC-085/CHG-090, inactive freeze/validation, exact sources and report, unchanged accepted authorities | Technically complete and review-ready uncommitted; commit/activation/claim/runtime/result remain closed |
| `P2-I2-CHG-092` | Owner requests one proper interpretative description of all P2-I2 cases and appendices after retaining APP-B3/B4 at `863d73c` | `p2_i2_retained_evidence_interpretative_synthesis_entry` | Add INT-H01 and section 22 first; map every main cell/mode and Appendix A/B stage through geometry, mechanism, bounded function, and prospective foundation; preserve accepted/technical/unrun status | Documentation and retained-evidence reading only; zero PyGRC/model/producer/arm/response; no reanalysis, new classification, recurrence, semantic relabel, or implementation priority | Accepted main P2-I2/I11, accepted Appendix A, retained APP-B2/B3 evidence, inactive APP-B4 freeze, owner request | INT-01 active; synthesis completion/acceptance/commit and every new runtime remain closed |
| `P2-I2-CHG-093` | INT-01 completes one evidence-linked interpretative synthesis | `p2_i2_all_cases_and_appendices_interpretative_synthesis` | Retain one causal rather than chronological account of common geometry, three modes, seven cells, perturbation/stability boundaries, Appendix A, APP-B1–B4, demonstrated functions, and prospective foundations | Eleven read-only `.venv` inspection/validation starts; exact checks bind 234 main terminals, 19 Appendix A arms, 99 APP-B2 arms, APP-B4 zero-runtime lifecycle, and 15/15 report links; zero PyGRC/model/producer/arm/response | DEC-087/CHG-092, retained reports/contracts, synthesis report, overview/brief navigation | Technically complete and review-ready uncommitted; no new scientific claim; owner acceptance/commit and APP-B4 runtime closed |
...
| `P2-I2-APP-A2-IMPLEMENTATION` | APP-A2 | [Entry freeze](../contracts/p2-i2/app-a2-entry-freeze.json), [inactive activation freeze](../contracts/p2-i2/app-a2-inactive-activation-freeze.json), [96/96 validation](../contracts/p2-i2/app-a2-inactive-static-validation.json), [runner](../scripts/p2_i2_app_a2_run.py), [analysis](../scripts/p2_i2_app_a2_analysis.py), [reconstructor](../scripts/p2_i2_app_a2_reconstruct.py), and [validator](../scripts/p2_i2_app_a2_validate.py) | Exact fresh-process 19-arm implementation and inactive boundary only | Owner-accepted and retained at `c435b00`; zero scientific runtime in construction |
| `P2-I2-APP-A2-ACTIVATION` | APP-A2 | [Activation authorization](../contracts/p2-i2/app-a2-activation-authorization.json), [owner acceptance](../contracts/p2-i2/app-a2-activation-owner-acceptance.json), [51/51 validation](../contracts/p2-i2/app-a2-activation-candidate-validation.json), and [validator](../scripts/p2_i2_app_a2_activation_validate.py) | One commit-bound campaign attempt and retained-only reconstruction authority | Owner-accepted and retained at `e61dacc`; one campaign claim consumed; zero retry |
| `P2-I2-APP-A2-RUNTIME` | APP-A2 | [19-arm aggregate](../contracts/p2-i2/app-a2-runtime-evidence.json), [read-only reconstruction and disposition](../contracts/p2-i2/app-a2-reconstruction-and-closeout.json), [53/53 closeout validation](../contracts/p2-i2/app-a2-closeout-validation.json), [closeout validator](../scripts/p2_i2_app_a2_closeout_validate.py), [owner acceptance and Appendix closeout](../contracts/p2-i2/app-a2-result-owner-acceptance-and-appendix-closeout.json), and [cumulative report](../reports/P2-I2-APP-A2-fresh-runtime-matrix-and-closeout.md) | One bounded positive runtime result: complete proper-subset insufficiency plus operation-specific and carrier causal paths; explicit plurality/order limitations and blocked claims | Owner-accepted under DEC-074/CHG-079; Appendix A closed; containing closeout commit authorized |
...
**Status:** closed and owner-accepted under DEC-074/CHG-079. APP-A2 completed
its single governed campaign and retained-only reconstruction: 19/19 arms are
valid, the primary and causal relations pass, and closeout validates 53/53.
One containing closeout commit is authorized; later work remains closed.

**Purpose:** test one exact extension of the accepted P2-I2 relation:

> Can physically distinct generator, extractor, and processor operations
> compose with subset-bounded irreducibility through one non-private carrier,
> relative to one frozen gate signature, so that a later response depends on
> the complete ordered composition rather than on any valid proper operation
> subset?

Appendix A inherits its operations from N29 I14.1, I14.2-3, I14.3, and the
producer-mediated I14.5-2 buffered bridge. It inherits N30's changed-medium to
later-eligibility relation and P2-I2's carrier factorization, attribution,
intervention, restoration, isolation, reconstruction, and claim controls.
It does not reinterpret N29/N30 as three-participant evidence.

The compact cycle is intentionally limited to:

```text
APP-A0 = source/semantic/hypothesis binding and owner review
APP-A1 = one combined native-first delta audit, exact realization/fixture
         registration, and execution freeze
APP-A2 = one bounded fresh-PyGRC-runtime complete-subset execution followed
         by deterministic reconstruction, control resolution, and one
         conclusion
```

Appendix A does not repeat I04–I11 calibration, null, registration ceremony,
separate control-resolution iteration, or separate reconstruction iteration.
Accepted P2-I2 machinery is reused byte-exactly where applicable; every
necessary delta is frozen once in APP-A1. N29's numerical gates may be reused
without tuning only after APP-A1 proves metric-semantic and measurement-domain
equivalence. The fixed `0.82` buffered-feedback retention factor is a
configuration identity, not an independent measured gate. APP-A2 returns one
cumulative evidence and closeout package for review. Artifact arithmetic,
replay-only evidence, or report assembly cannot satisfy APP-A2.

APP-A1 has this internal hard order while remaining one named stage:
...
No implementation observation may retrospectively select a design, value,
threshold, or control.

```text
iteration_id = P2-I2-APP-A
entry_authority = owner direction to retain the extension inside P2-I2 as
                  Appendix A and simplify the cycle around evidence of the
                  exact composition claim
current_scope = semantic/hypothesis/checklist/decision construction only
source_operations = G generative enrichment; E leakage-gated extraction;
                    P processor redistribution/buffering; R later generator
                    response to the changed medium
primary_matrix = reference, G, E, P, GE, GP, EP, GEP
nearest_comparison_family = GE, GP, EP considered together without scalar
                            ranking or outcome-selected strongest pair
gate_signature = four measured gates: environment feedback; support feedback;
                 phase residual; merge leakage; plus fixed retention
                 configuration identity required identically in every arm
proper_subset_rule = every valid member of reference/G/E/P/GE/GP/EP satisfies
                     the same retention configuration but fails at least one
                     of the four measured gates; configuration drift makes an
                     arm nonevaluable and cannot establish insufficiency
operational_validity = registered live-carrier invocation plus matched
                       opportunity/cost/timing/path and complete receipts;
                       recorded carrier effect may be positive, negative, or
                       zero without invalidating the arm; reference requires
                       exact baseline plus verified diversion and matched
                       participant activity/receipts
transportability_gate = metric_semantics_equivalent AND
                        measurement_domain_equivalent; otherwise N29 values
                        are diagnostic context only and execution freeze fails
transportability_dimensions = raw definitions; units/orientation; reference/
                        candidate meanings; windows; normalization/
                        denominators; topology/scaling; leakage accounting;
                        phase residual; R target; runtime numeric domain
participant_projection = S_G performs G, S_E performs E, S_P performs P;
                         one later noncontributor R reads only the carrier
required_scope_diagnostic = one capacity-relaxed source performs GEP; success
                            makes plurality non-load-bearing; failure alone
                            leaves plurality unresolved
role_identity_guard = prove matched capacity/access/authority/timing/
                      initialization/cost and freeze at least one complete-GEP
                      contributor-to-operation role rotation; APP-A2 must
                      reproduce the complete relation and operation-to-R paths
                      before any bounded plurality-load-bearing classification
required_causal_controls = missing operations diverted with activity/cost/
                           timing/opportunity matched; each G/E/P withdrawal
                           changes R directly or changes a registered mediator
                           whose independent intervention changes R; carrier
                           freeze/clamp changes R; direct/private/controller
                           read exclusion
required_order_controls = E->G->P and G->P->E adjacent inversions; both are
                          needed for a complete ordered-sequence claim; one
                          alone supports only some handoff order-sensitivity
reused_guards = repository .venv only; no absolute paths; exact restoration;
                fresh-process isolation; deterministic reconstruction;
                accepted claim ceilings
required_runtime_evidence = one fresh process/model/baseline per arm; live
                source-current PyGRC carrier; live selected-G/E/P invocations
                and R response; exact predecessor-to-successor carrier-
                identity continuity; operation/event/packet/intervention/
                restoration receipts; deterministic readback reconstruction
evidence_effect_now = none
forbidden_now = capability/runtime audit; implementation selection; fixture
                construction; calibration; execution; Appendix A result;
                cooperation/coordination/resource-economy claim; N31+
exit_now = uncommitted semantic package returned for owner review
```

Checklist:

- [x] Record Appendix A as a named P2-I2 activity before constructing its
  semantic appendix.
- [x] Declare its subordinate operational hypothesis before any capability,
  realization, fixture, or runtime activity.
- [x] Bind the exact N29/N30/P2-I2 provenance and producer/native debts.
- [x] Define the simplified APP-A0–APP-A2 cycle and prohibit a duplicate
  I00–I11 process.
- [x] Preserve the accepted P2-I2 result and all main-lane gate states.
- [x] Apply the owner-supplied APP-A0 critical review without starting a new
  iteration: gate-vector conjunction, valid scientific zeros, operation-to-`R`
  causal paths, threshold transportability, asymmetric plurality, adjacent
...
- [x] Execute the complete eight-subset matrix and bounded diagnostics once
  ...
  deterministically, resolve the exact claim, and stop.
...
retention commit. That retention later succeeded at `1f69816`; APP-A2 is now
complete and separately governed by DEC-069–073/CHG-074–078.
...
**Status:** result owner-accepted and Appendix A closed under DEC-074/CHG-079;
one containing closeout commit authorized.
...
- [x] Commit the accepted activation package (`e61dacc`), verify the exact clean committed
  HEAD, and invoke the one governed campaign through `.venv`.
- [x] If activated, execute exactly 19 arms once, fail closed on any missing or
  nonevaluable arm, and retain only the one aggregate runtime artifact.
- [x] Reconstruct and interpret only from that retained aggregate; validate the
  exact conclusion and stop for owner review without an unreviewed commit.
- [x] Validate the aggregate, reconstruction, bounded conclusion, process
  counts, portability, and zero-regeneration boundary; retain one 53/53
  closeout-validation artifact.
- [x] Obtain explicit owner acceptance of the exact bounded result, close
  Appendix A, and authorize one containing closeout commit.
- [x] Run one `.venv`-bound, non-scientific acceptance/identity integrity
  check over the exact reviewed hashes and closeout record.
- [x] Retain the complete accepted APP-A2 result and Appendix A closeout in the
  one authorized commit.

Preactivation APP-A2 construction counts:
...
Preactivation isolation closeout:
...
Historical activation-candidate counts before DEC-072 acceptance:
...
APP-A2 runtime and closeout counts:

```text
activation commit = e61dacc
campaign claims consumed = 1
campaign invocations = 1
campaign retries = 0
child arm starts = 19
child arm retries = 0
...
per-arm persisted files = 0
aggregate runtime artifacts = 1
reconstruction invocations = 1
retained aggregate reads = 1
analysis reconstruction byte-identical = true
reconstruction PyGRC/model/child/producer/scientific regeneration = 0/0/0/0/0
closeout validator starts = 3 (52/53 check-only; corrected 53/53 check-only
                              + retained)
closeout validator diagnostics = 1
closeout validation = 53/53 passed
closeout-validation PyGRC/model/child/producer/scientific regeneration = 0/0/0/0/0
Appendix results = 1
terminal classification = supported_bounded_candidate
owner result acceptance = granted under DEC-074/CHG-079
Appendix A status = closed
result commit = authorized and pending
acceptance-integrity Python starts = 3 (8/9 initial; one diagnostic;
                                        corrected 9/9 final)
initial-only mismatch = lexical sys.executable versus resolved `.venv` path
acceptance-integrity PyGRC/model/child/producer/scientific regeneration = 0/0/0/0/0
```
...
all arms operationally valid = true
all seven proper subsets insufficient = true
full GEP gate signature passed = true
primary relation passed = true
G, E, and P direct-withdrawal/mediator-restoration paths passed = true
carrier-clamp causal relation passed = true
load-bearing external producer = false
one-source reproduction = true
cyclic role-rotation reproduction = true
label invariance = true
tested EGP and GPE adjacent inversions pass = true
physical participant plurality load-bearing = false
tested operation order load-bearing = false
...
```text
clean committed preflight = passed
campaign claims consumed = 1
campaign parent starts = 1
child starts = 1
failed child arm = primary:state_carried:reference:seed-101
child exit code = 1
retained child receipts = 0
later child starts = 0
governed runtime aggregates = 0
per-arm outputs = 0
child stderr/final failure receipts = 0/0
child PyGRC/model/packet/producer/response counts = unobservable
Appendix B scientific results = 0
replacement authority = absent
```

Evidence: [APP-B2 failed-start report](../reports/P2-I2-APP-B2-failed-start.md),
`app-b2-campaign-claim.json`, and `app-b2-failed-start.json`.

APP-B2 bounded recovery checklist:

- [x] Retain owner authorization before editing the runner or constructing a
  replacement authority.
- [x] Replace resolved/global child launch with lexical `.venv/bin/python` and
  mechanically refuse any non-venv child environment before PyGRC import.
- [x] Persist one aggregate failure receipt for any nonzero/malformed child,
  including exit/stderr digest and redacted diagnostic, before continuing the
  remaining frozen rows.
- [x] Preserve the original claim and failed-start bytes permanently; use
  distinct replacement claim/output identities.
- [x] Candidate-free validate exact scientific-byte invariance, venv-only
...
validator-only live lexical-.venv child probes = 3
final grouped validation checks = 12 passed
unchanged scientific runtime function digests = 10/10
PyGRC imports / models / scientific arms / response calls = 0/0/0/0
original claim/failure hash checks = passed
replacement claim/output absence = passed
replacement campaigns started = 0
```
...
### 21.4 `P2-I2-APP-B3` — Pre-acceptance implication-boundary audit

- [x] Record the owner review as DEC-081/CHG-086 before running the audit.
- [x] Prove whether a frozen three-admission, quantity/timing/history-length-
  matched alternative to GEP exists; do not retrofit one if absent.
- [x] Prove the original fail-open projection was post-run only and could not
  affect arm scheduling, continuation, retry, receipt retention, or outcomes.
- [x] Derive corrected validity from native identities, registered routes and
  amounts, queue/contamination/restoration receipts, never authored validity.
- [x] Reconcile expected physical G/E/P admissions to actual H_C provenance
  one-to-one across all 99 arms, with zero unexpected and duplicate admissions.
- [x] Prove materialization, neutral, response, controller, diversion, state-
  intervention, private-partition, and save/load/reset activity cannot enter
  H_C.
- [x] Freeze the strongest positive wording as bounded history-carried P2-I2-
  grounded three-operation shared-pool composition candidate with
  `support_status=scaffold_dependent`.
- [x] Retain state/hybrid negatives as frozen-realization statements and record
  the hybrid result as causal rather than an implementation failure.
- [x] Return one uncommitted audit artifact/report with zero PyGRC/model/
  producer/arm/response generation.

APP-B3 result:

```text
matched three-admission alternative registered = false
operation complementarity = unresolved / not claimed
fail-open analysis stage = after all 99 child attempts
arms retried, removed, or outcome-reclassified = 0
corrected authored-valid reads = 0

expected/actual physical H_C admissions = 123/123
G/E/P admissions = 41/41/41
unexpected/duplicate admissions = 0/0
route-or-event provenance mismatches = 0
explicitly cleared admissions = 12
expected/actual final active tokens = 111/111
excluded-activity admissions = 0

strongest positive claim = bounded history-carried P2-I2-grounded
                           three-operation shared-pool composition candidate
support_status = scaffold_dependent
APP-B3 PyGRC/models/producers/arms/responses = 0/0/0/0/0
validator starts = 6 (2 check-only, 4 artifact generation)
superseded uncommitted audit artifacts = 3
```

Evidence: [APP-B3 implication-boundary audit](../contracts/p2-i2/app-b3-implication-boundary-audit.json),
[validator](../scripts/p2_i2_app_b3_validate.py), and
[audit report](../reports/P2-I2-APP-B3-implication-boundary-audit.md).

### 21.5 `P2-I2-APP-B4` — Ordered three-token discriminator

- [x] Record DEC-083/CHG-088 and APP-B4-H01 before capability work.
- [x] Audit unchanged-baseline native coherence budgets and route reuse for all
  27 ordered G/E/P triples without constructing a model or response.
- [x] Prove whether every triple can execute using the accepted native
  operation amounts, three slots, topology, and history admission unchanged.
- [x] If any triple is infeasible, stop before freeze/runtime and return exact
  provisioning versus reduced-panel choices; do not silently choose.
- [x] Record owner selection of the unchanged-baseline feasible panel before
  freeze construction.
- [x] Freeze exactly 24 feasible triples plus reference across three seeds as
  one 75-arm history-only campaign with fresh process/model per arm.
- [x] Reuse accepted response, history, restoration, contamination, one-shot,
  cumulative-output, and retained-only reconstruction machinery.
- [x] Classify raw cardinality, dose-multiset, and within-multiset ordering
  separately; never infer operation complementarity from this panel.
- [x] Return the uncommitted freeze or capability blocker for owner review
  before any live APP-B4 campaign.

APP-B4 entry counts:

```text
capability/budget audit starts = 2 (1 check-only, 1 retained output)
ordered triples audited / feasible / infeasible = 27/24/3
infeasible = EEG, EEE, EEP
frozen triples / arms = 24/75
...
campaign claims / parents / children = 0/0/0
PyGRC imports / models / producers / arms / responses = 0/0/0/0/0
APP-B2 or APP-B3 bytes changed by APP-B4 freeze construction = 0
```

Owner choices:

```text
A (recommended): unchanged baseline, 24 triples + reference x 3 seeds = 75 arms
B: uniform native P pre-funding 0.5625, full panel + anchors = 87 arms
C: 84-arm registry with 3 preregistered operationally infeasible rows
```

Evidence: [APP-B4 capability audit](../contracts/p2-i2/app-b4-unchanged-baseline-capability-audit.json),
[capability validator](../scripts/p2_i2_app_b4_capability_audit.py),
[capability report](../reports/P2-I2-APP-B4-capability-audit.md),
[inactive freeze](../contracts/p2-i2/app-b4-inactive-75-arm-freeze.json),
[inactive validation](../contracts/p2-i2/app-b4-inactive-freeze-validation.json),
[freeze validator](../scripts/p2_i2_app_b4_validate.py), and
[inactive-freeze report](../reports/P2-I2-APP-B4-inactive-freeze.md).

APP-B4 completion re-entry:

- [x] Record DEC-089/CHG-094 before activation work.
- [x] Audit the frozen runner's activation and clean-commit binding before
  constructing an activation artifact.
- [x] Preserve the committed inactive freeze and record the exact
  self-referential-HEAD activation blocker.
- [x] Correct only activation/commit binding using tracked byte identity,
  immutable implementation-commit ancestry, and invocation-HEAD equality.
- [x] Construct one prospective activation artifact with exact command, paths,
  claim, output, graph, `.venv`, and zero-retry identities.
- [x] Candidate-free validate the correction and activation while owner
...
```text
read-only source/data inspection starts = 3
syntax/static-source inspection starts = 3
activation builder starts = 1
activation validator starts = 4 (1 failed before science, 3 passed)
inactive runner-refusal starts = 1
total repository `.venv` preparation starts = 12
owner-acceptance digest/validation starts after readiness = 2
scientific freeze rows changed = 0
activation / claim / runtime / reconstruction = 0/0/0/0
...
[candidate-free validation](../contracts/p2-i2/app-b4-activation-candidate-validation.json),
...
## 22. `P2-I2-INT-01` — Interpretative synthesis of all cases and appendices

- [x] Record DEC-087/CHG-092 and INT-H01 before synthesis work.
- [x] Describe the common geometry from source contributions through carrier,
  response mask, native producer, and later B response.
- [x] Describe state-carried, history-carried, and hybrid cases separately,
  without ranking or collapsing them.
- [x] Explain all seven main logical cells and the geometric alternative each
  removes, preserves, or substitutes.
- [x] Explain the perturbation, stability, restoration, access, attribution,
  private-partition, quantity-matched, and controller results.
...
- [x] Mark every statement as retained observation, warranted interpretation,
  or prospective possibility; do not convert the latter into evidence.
...
new PyGRC imports / models / producers / arms / responses = 0/0/0/0/0
new scientific analysis or result assignment = 0
APP-B4 activation / claim / runtime = 0/0/0
read-only `.venv` inspection and validation starts = 11
validated main / Appendix A / APP-B2 arm counts = 234/19/99
validated synthesis links = 15/15
```

Evidence: [interpretative synthesis](../reports/P2-I2-interpretative-synthesis.md),
[main terminal closeout](../reports/P2-I2-I11-terminal-closeout.md),
[Appendix A result](../reports/P2-I2-APP-A2-fresh-runtime-matrix-and-closeout.md),
and [Appendix B result](../reports/P2-I2-APP-B2-result.md).
