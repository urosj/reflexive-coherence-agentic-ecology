...
**RCAE iteration at handoff:** `P2-I3-I03`
...
This document transfers one precise substrate question from Agentic Ecology
back to the graph/LGRC project and records exactly how P2-I3 must resume after
that question is investigated.

The handoff exists because P2-I3 reached a genuine graph/ecology spiral point:
...
This is not a failure of P2-I3. It is the intended downward part of the
cross-project spiral: ecology demand exposes a missing graph distinction, and
the graph project tests it without borrowing ecology demand as evidence.

## 2. Authority and repository boundary

At this handoff point:

- the RCAE repository owns this demand report, the pause boundary, and the
  later return/admission procedure;
- the graph repository owns N31 theory-to-runtime investigation, native PyGRC
  changes, graph experiment evidence, and any graph-side specification;
- the current RCAE working scope keeps the graph and geometric-theory
  repositories read-only;
- no RCAE command may edit, stage, commit, or branch the graph repository;
- graph work begins only after an explicit workspace/scope switch; and
- returning from N31 does not authorize RCAE to rewrite graph evidence.

The repositories must remain independently reproducible. Machine-specific
PyGRC checkout or environment locations remain local configuration and must
not be committed into RCAE artifacts.

## 3. P2-I3 state frozen at departure
...
- DEC-021 selected route-exclusive intermediate-node coherence as the minimum
  carrier under then-admitted capabilities;
...
### 3.3 Questions intentionally left open

```text
P2-I3-Q-005  selected non-static dynamic
P2-I3-Q-006  equation, units, update order, invariants
P2-I3-Q-007  local traversal encounter
P2-I3-Q-008  current-state versus active-history mode
P2-I3-Q-013  quantity/state-matched operational identities
P2-I3-Q-015  restoration and equal-continuation design
all I04-and-later questions
```

No placeholder answer may be inserted while N31 is active.

### 3.4 Work that must not occur during the handoff
...
- open calibration preregistration;
- generate a matched null;
- register or execute a candidate matrix;
- classify the handoff as a negative L03 result; or
- treat a graph work-in-progress observation as admitted source evidence.

Documentation may preserve questions or clarify this handoff, but it may not
advance the discriminator gate.

## 4. Graph-side question selected by the demand pass
...
direction. The later cross-project addendum deliberately leaves N31 unfixed
until Agentic Ecology supplies a demand. This handoff selects the broader
decay-semantic question rather than assuming conductance is already the
answer.

The graph project may refine the display name while preserving the returned
identity and scope.
...
- system-internal rather than wall-clock time;
- local causal action rather than global ranking;
- minimal hidden continuation state;
- complete snapshot/load/reset/branch identity;
...
-> a registered internal age/phase changes future packet formation
-> newly created packets carry less coherence
```

Mandatory boundary:

- packet amount may be selected at creation;
- an in-flight packet may not attenuate;
- source debit and target credit use the same created amount;
- unreleased coherence remains at the source; and
- any age/phase state is explicit continuation state.

N31 should determine whether this is a true field-decay primitive or only a
...
-> native packet departure debits the source
-> in-flight state carries the exact amount
-> a declared destination receives the exact amount
-> local source field magnitude decreases
```

Mandatory boundary:

- local loss has an explicit destination;
- total node-plus-in-flight coherence remains invariant;
- emission uses internal LGRC time and source state;
- the receiver cannot be omitted from topology or continuation identity;
...
-> that state affects later local transport
-> without reinforcement it relaxes toward baseline
```

Mandatory boundary:

- current theory supports instantaneous state/flux-derived geometry but does
  not already define slow susceptibility;
- N08 records native route-conductance memory as missing;
- N22 supplies producer-mediated conductance precedent, not native authority;
- susceptibility is not coherence mass and needs its own bounds/invariants;
...
Each candidate receives the smallest topology that faithfully expresses its
carrier, invariant, local action, and restoration state. Cross-candidate
comparison should use semantic criteria and normalized evidence roles, not
raw effect magnitudes from nonequivalent fixtures.
...
Every candidate claimed as decay must demonstrate:

```text
declared state or relation formed
-> forming activity stops
-> system-internal progression occurs
-> declared state or relation changes autonomously
-> a local readout changes because of that state
-> intervention on the mediator changes/removes the readout effect
...
- instantaneous geometry from slow relational aftereffect;
- internal time from wall time;
- native transition from producer policy;
- current-state sufficiency from hidden active history;
- local encounter from route ranking;
- implementation conformance from scientific evidence; and
- candidate usefulness from native admission.
...
- current state installed without formation history;
- complete-state-matched history contrast where causally meaningful;
- time/phase intervention;
- destination/accounting audit for any moved coherence;
- local-readout versus global-selector exclusion;
- snapshot/load/branch equal continuation;
...
new missing distinction exposed
```

Selection is not mandatory.

## 7. Required N31 return bundle

RCAE must not resume from a prose conclusion alone. N31 should return one
manifest that maps the following logical roles to exact graph-repository
artifacts. Filenames may follow graph-project conventions; the roles are
mandatory unless explicitly inapplicable with a reason.

### 7.1 Identity and source roles
...
- claim-boundary index entry or equivalent.
...
- pre-execution equations, parameters, and update order;
- conformance records separate from scientific runs;
- finite execution matrix and attempt/retry records;
- state, event, packet, timing, and readout traces;
- mediator interventions and control outcomes;
- snapshot/load/reset/branch continuation evidence;
- exact invariant audits;
- reconstruction record; and
- developmental interpretation of narrow, mixed, or negative results.
...
- implementation source and unit/integration tests;
- backward-compatibility and side-effect boundary;
- producer-to-native transition/equivalence comparison where a producer
  predecessor exists;
- explicit statement that historical RCAE code does not switch to the native
...
The final N31 handoff record should expose at least:
...
invariants_by_primitive
topology_scope_by_primitive
native_api_by_primitive
producer_residue_by_primitive
naturalization_debt_by_primitive
...
All paths inside retained artifacts must be repository-relative or use an
...
P2-I3 owns the later ecology composition and controls.
...
6. Switch working scope to the graph repository only after explicit owner
   authorization.
7. Create the N31 graph branch from a clean admitted graph base.
8. Reference the RCAE handoff by repository-relative locator plus commit, not
   by copying mutable conversation text.

The current instruction not to edit the graph repository from the RCAE
project remains active until step 6.

## 10. Return preflight after N31 closes

P2-I3 must resume from a clean, traceable state.

### 10.1 Verify closure rather than work-in-progress state

Before changing RCAE:

- N31 has a retained terminal or explicit bounded-incomplete closeout;
- its graph branch changes are committed;
- the graph worktree is clean;
- the return manifest exists and maps all required logical roles;
- every required reproduction command is documented;
- native versus producer-assisted status is explicit; and
- N31 states what P2-I3 may and may not consume.

An interesting intermediate result is not enough.

### 10.2 Reconstruct N31 in graph scope

Run the exact N31 reconstruction and tests in the graph repository before
RCAE admission. Record:
...
- any omitted large-artifact regeneration procedure; and
- whether reconstruction was byte-exact or semantically exact under a named
  canonicalization contract.

Failed reconstruction prevents RCAE source transition. It is not a P2-I3
scientific negative.
...
## 11. RCAE source-transition procedure

N31 does not silently replace the I02 source bundle. P2-I3 must perform a
bounded source transition before reopening realization work.

### 11.1 Retain the historical source bundle

Keep the accepted I01/I02 artifacts and their original graph revision intact.
Do not regenerate them in place or change their historical classifications.

### 11.2 Add an N31 return admission package

Create a new P2-I3 return/admission iteration containing:

- exact N31 graph revision and PyGRC version;
- return-manifest digest;
- changed/new source files and public callables;
- changed/new tests;
- specification identities;
- candidate disposition and claim ceiling;
- reproduction results;
- compatibility with DEC-020 LGRC9V3 authority;
- effect on DEC-021 carrier choice;
- effect on DEC-022 ownership rows; and
- explicit non-effects on P2-I1, P2-I2, and historical P2-I3 evidence.

This is a source transition, not a rerun of the original broad I01 audit,
unless N31 changed unrelated load-bearing runtime surfaces.
...
```text
likely P2-I3 effect:
  Q-005 may select native conserved leakage
  DEC-021 node carrier may remain adequate
  DEC-022 leakage row must reopen from RCAE producer to native operation
  Q-006 consumes but independently registers the N31 law and parameters
```

Required checks:

- P2-I3 topology includes the complete native destination/accounting surface;
- native leakage acts autonomously under registered internal time;
- probe and lifecycle packets remain distinct;
...
likely P2-I3 effect:
  Q-003 must reopen
  DEC-021 node carrier may become a formation input rather than field carrier
  DEC-022 ownership must reopen
  Q-005 may select native relational relaxation
```
...
- which primitive directly answers L03's minimum discriminator;
- whether the other is a control, variation, later campaign, or unnecessary;
- whether DEC-012's proportional default still applies; and
- whether combining the primitives would create a third untested mechanism.
...
- explicitly adopt the exact producer behavior under a new RCAE realization
  identity;
- construct a different ecology-side mechanism while preserving N31's debt;
- keep Q-005 deferred; or
- close/redirect the lane as missing native substrate.

Producer evidence must not be upgraded to native. The original RCAE two-
...
- narrow L03 to another accepted non-static dynamic such as reinforcement or
  saturation through a new recorded decision; or
- close the current realization as a redirect without claiming an L03
  scientific negative.
...
new primitive into an old label for convenience.

## 13. Decision sequence on return

After the N31 source transition passes, resume in this order:

1. Accept or reject the N31 return bundle for P2-I3 consumption.
...
11. Continue to I04 calibration preregistration under the existing checklist.

The exact future decision IDs are assigned when the return occurs. This
handoff does not reserve them or predetermine their answers.

## 14. What N31 evidence can and cannot discharge
...
| L03 terminal classification | No | N31 is substrate evidence only |
...
Stop and ask for owner direction if:

- the N31 return manifest is missing or cannot reconstruct;
- the graph revision differs from the retained N31 identity;
...
These are decision or infrastructure boundaries, not scientific negatives.

## 17. Completion condition for this handoff

This handoff is complete when:

```text
RCAE demand package committed
+ N31 executed and closed in graph scope
+ N31 return bundle independently reconstructs
+ bounded RCAE source transition accepted
+ affected carrier/ownership decisions explicitly retained or reopened
+ Q-005 explicitly resolved
= P2-I3 I03 may resume
```

Until then:

```text
P2-I3 status = intentionally paused at cross-project graph spiral
P2-I3-DISCRIMINATOR-GATE = pending
P2-I3 scientific result = none
N31 demand selection = authorized by RCAE owner direction
graph implementation authority = graph-project scope only
```
