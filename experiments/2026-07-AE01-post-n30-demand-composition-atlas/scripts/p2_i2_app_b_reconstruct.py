#!/usr/bin/env python3
"""Pure reconstruction and closeout projection for Appendix B."""
...
def main() -> int:
    ...
    require(digest_value(without_digest) == retained["canonical_payload_digest"], "retained payload digest drifted")
    require((ROOT / args.input).read_bytes() == canonical_bytes(retained), "retained output is not canonical")
    ...
    require(reconstructed == retained["analysis"], "analysis reconstruction differs")
    result = {
        "artifact_id": "P2-I2-APP-B2-RECONSTRUCTION-AND-CLOSEOUT",
    ...
    require(not output.exists(), "closeout output already exists")
    output.write_bytes(canonical_bytes(result))
    require(load_json(output) == result, "closeout readback drifted")
    print(json.dumps({"status": "reconstructed", "terminal": reconstructed["terminal_classification"]}))
    return 0
